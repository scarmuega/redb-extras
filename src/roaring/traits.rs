...
mod tests {
    ...
    fn test_empty_bitmap_size() {
        ...
        let size = handler.get_value_size(&bitmap).unwrap();
        // Should include at least the version byte and whatever the empty roaring struct has. That adds up to 9.
        // TODO: see if there's a more strict way of anticipating the expected length, this seems to implementation-specific.
        assert_eq!(size, 9);
    }
}
